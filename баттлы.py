...
BACKGROUND = (20, 20, 35)
GRID_COLOR = (40, 40, 60)
PLAYER_COLORS = [(0, 255, 170), (255, 100, 100)]
...
class Game:
    ...
    def start_game(self):
        self.cell_size = min(HEIGHT // (self.board_size + 4), 16)
    ...
    def update_valid_positions(self):
        ...
        # Для первого хода - только углы
        if self.first_move[self.current_player]:
            # Первый игрок ставит в левый верхний угол
            if self.current_player == 0:
                self.valid_positions = {(0, 0)}
            # Второй игрок ставит в диагонально противоположный угол
            else:
                x = self.board_size - self.current_rect.width
                y = self.board_size - self.current_rect.height
                self.valid_positions = {(x, y)}
            return
    ...
    def can_place(self, x, y):
        ...
        # Новая механика: как минимум одна сторона должна перекрываться с уже стоящими фигурами
    ...
    def place_rect(self, x, y):
        ...
        self.rotation = 0  # Сброс поворота для следующего игрока
        self.skip_turn_available = False
        self.roll_dice()
    ...
    def end_game(self):
        ...
        self.player_scores = [player1_score, player2_score]
    ...
    def bot_move(self):
        ...
        # Простая стратегия бота: ставить фигуру как можно ближе к противнику
        best_pos = None
        min_distance = float('inf')
        
        # Ищем позицию, ближайшую к фигурам противника
        for pos in self.valid_positions:
            x, y = pos
            distance = self.get_distance_to_opponent(x, y)
    ...
    def draw_menu(self, screen):
        ...
        for i, (text, size) in enumerate(sizes):
            rect = pygame.Rect(WIDTH//2 - 150, 250 + i*100, 300, 70)
            color = SELECTED_COLOR if size == self.selected_size else UI_BORDER
            pygame.draw.rect(screen, UI_BG, rect, border_radius=15)
        ...
        for i, (text, mode) in enumerate(modes):
            rect = pygame.Rect(WIDTH//2 - 150, 600 + i*80, 300, 60)
        ...
        if self.selected_size and self.selected_mode:
            start_rect = pygame.Rect(WIDTH//2 - 100, 770, 200, 60)
    ...
    def draw_board(self, screen):
        ...
        for player_idx in [0, 1]:
            for rect_data in self.placed_rects[player_idx]:
                x, y, w, h = rect_data
                pygame.draw.rect(
                    screen, PLAYER_COLORS[player_idx],
        ...
        if self.game_over:
            ...
            score_text = font.render(f"Счет: {self.player_scores[0]} - {self.player_scores[1]}", True, TEXT_COLOR)
    ...
    def draw_ui(self, screen):
        ...
        # Панель информации
        info_rect = pygame.Rect(60 + board_width, 50, WIDTH - board_width - 70, 400)
        ...
        player1_text = font.render(f"Игрок 1: {player1_score} клеток", True, PLAYER_COLORS[0])
        player2_text = font.render(f"Игрок 2: {player2_score} клеток", True, PLAYER_COLORS[1])
        screen.blit(player1_text, (info_rect.x + 20, info_rect.y + 20))
        screen.blit(player2_text, (info_rect.x + 20, info_rect.y + 60))
        
        # Текущий игрок
        if not self.game_over:
            current_text = font.render(f"Сейчас ходит: Игрок {self.current_player + 1}", True, PLAYER_COLORS[self.current_player])
            screen.blit(current_text, (info_rect.x + 20, info_rect.y + 110))
            
            # Режим игры
            mode_text = font.render(f"Режим: {self.game_mode}", True, TEXT_COLOR)
            screen.blit(mode_text, (info_rect.x + 20, info_rect.y + 140))
            
            # Результаты кубиков
            dice_text = font.render(f"Кубики: {self.dice_result[0]} и {self.dice_result[1]}", True, TEXT_COLOR)
            screen.blit(dice_text, (info_rect.x + 20, info_rect.y + 170))
            
            # Размер прямоугольника
            rect_text = font.render(f"Прямоугольник: {self.current_rect.width} x {self.current_rect.height}", True, TEXT_COLOR)
            screen.blit(rect_text, (info_rect.x + 20, info_rect.y + 200))
            
            # Предложение пропустить ход
            if self.skip_turn_available:
                skip_text = font.render("Нет ходов! Нажмите SPACE чтобы пропустить", True, WARNING_COLOR)
                screen.blit(skip_text, (info_rect.x + 20, info_rect.y + 230))
            
            # Кнопка вращения
            rotate_rect = pygame.Rect(info_rect.x + 20, info_rect.y + 270, info_rect.width - 40, 40)
            ...
            # Инструкция для первого хода второго игрока
            if self.first_move[1] and self.current_player == 1:
                first_move_text = font.render("Первый ход: диагонально противоположный угол", True, WARNING_COLOR)
                screen.blit(first_move_text, (info_rect.x + 20, info_rect.y + 320))
        else:
            # Информация о победителе
            if self.winner is not None:
                win_text = font.render(f"Победил Игрок {self.winner + 1}!", True, WIN_COLOR)
                screen.blit(win_text, (info_rect.x + 20, info_rect.y + 110))
            else:
                win_text = font.render("Ничья!", True, WIN_COLOR)
                screen.blit(win_text, (info_rect.x + 20, info_rect.y + 110))
            
            final_score = font.render(f"Финальный счет: {self.player_scores[0]} - {self.player_scores[1]}", True, TEXT_COLOR)
            screen.blit(final_score, (info_rect.x + 20, info_rect.y + 150))
            
            restart_text = font.render("ESC - в меню", True, TEXT_COLOR)
            screen.blit(restart_text, (info_rect.x + 20, info_rect.y + 200))
